...
| 6 | ✅ **ZROBIONE** (0.21.0) — własna zakładka `GRAFIK` w `TABS` (`employeeSessionShared.tsx`, wspólna dla kiosku i konta osobistego), pionowa lista dni, "Z tobą" na zmianie, URP/NIE z `absences`, przełącznik tydzień/następny/miesiąc, "Pokaż wszystkich w lokalu", kafelek "Następna zmiana" na Pulpicie. Widoczne WYŁĄCZNIE zmiany opublikowane (`publishedShiftsFor` w `utils/grafik.ts` — jedno miejsce filtrowania, nie w każdym dashboardzie osobno) | tak |
| 7 | ✅ **ZROBIONE** (0.22.0) — `utils/swaps.ts` (jedyne miejsce piszące do `shift_swaps` i przepisujące zmianę na nowego pracownika), u pracownika w zakładce Grafik: "Wystaw na giełdę" / "Wycofaj" / sekcja "Giełda — wolne zmiany"; u kierownika kolejka w `ZatwierdzanieZmian.tsx` **oraz** ✓/✗ w komórce siatki (tylko w trybie Edycja, ostrzeżenie widać też w Podglądzie). Limit `SWAP_MIN_HOURS = 12`. Przejęcie sprawdza wolne i kolizję godzin; niezgodne stanowisko nie blokuje | tak |
